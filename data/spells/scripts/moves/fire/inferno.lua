...
local function spellCallback(uid, position, damage, count)
	...
	if creature then		
		if count <= #area then
			...
			doAreaCombatHealth(uid, COMBAT_FIREDAMAGE, position, area[count], 0, 0, effect[count])
			doAreaCombatHealth(uid, COMBAT_FIREDAMAGE, position, areaDamage[count], -damage, -damage, 0)
			count = count + 1			
