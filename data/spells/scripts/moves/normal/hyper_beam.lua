...
local effects = {}
table.insert(effects, 585) --dir1
table.insert(effects, 586) --dir2
table.insert(effects, 585) --dir3
table.insert(effects, 586) --dir4

local function spellCallback(uid, position, positionDamage, damage, count, dir)
	local creature = Creature(uid)
	if creature then		
		if count <= waves then			
			doAreaCombatHealth(uid, COMBAT_NORMALDAMAGE, position, area, 0, 0, effects[dir+1])
			doAreaCombatHealth(uid, COMBAT_NORMALDAMAGE, positionDamage, areaDamage, -damage, -damage, 0)
			count = count + 1
