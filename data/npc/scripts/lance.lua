...
local nextPosition = Position(194, 1071, 7)
local pokemons = 
{
	[1] = 
		{
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') then
		local player = Player(cid)
		if player then
			selfSay('Seu desafio na Liga termina aqui e agora!', cid)
			...
			if npcHandler:isFocused(cid) and npcHandler.topic[cid] == 1 then
				...
				if not monster then
					if pokemons[duelStatus] then
						...
						selfSay('E isso ai! Eu odeio admitir isso, mas voce pode se tornar um mestre de Pokemon, ' .. player:getName() .. ". Apesar de ter derrotado a Elite Four, voce ainda possui um desafio: enfrentar o atual campeao. Va em frente e boa sorte!", cid)
						player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
