	...
	elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') then
		...
		if player then
			if player:getStorageValue(storage) > 0 then
				selfSay('O que esta fazendo aqui? Ja pegou seu premio!', cid)
				sendToExit(cid)
			else
				selfSay('Ambos sabemos o resultado... Mas como realmente quer tentar....', cid)
			...
			if npcHandler:isFocused(cid) and npcHandler.topic[cid] == 1 then
				...
				if not monster then
					if pokemons[duelStatus] then
						...
						selfSay('NAO! IMPOSSIVEL! Voce derrotou meu melhor time. ' .. player:getName() .. ", embora eu nao goste de admitir, voce e o novo campeao da Liga. Meus parabens!", cid)
						player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
						player:teleportTo(exitPosition)
						player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
