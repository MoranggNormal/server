...
local nextPosition = Position(239, 1071, 7)
local pokemons = 
{
	[1] = 
		{
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') then
		local player = Player(cid)
		if player then
			selfSay('Por favor, depois que perder mantenha a cabeca fria.', cid)
			...
			if npcHandler:isFocused(cid) and npcHandler.topic[cid] == 1 then
				...
				if not monster then
					if pokemons[duelStatus] then
						...
						selfSay('... As coisas nao deveriam terminar assim! Voce e melhor do que eu pensava, ' .. player:getName() .. ". Va em frente. Voce so teve um gostinho do poder da Liga Pokemon.", cid)
						player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
